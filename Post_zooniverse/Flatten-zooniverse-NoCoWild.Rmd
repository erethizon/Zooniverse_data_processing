...
### Turn on packages, bring in r scripts, set workspace and bring in data 
...
library(tidyr)
library(lubridate)
Filepath<-("/Volumes/External Drive/Dropbox/R/Zooniverse_data_processing/Post_zooniverse")
#you may need to change the file path from your computer, but you should be in the "Post_zooniverse" folder
...
classifications_file<- paste0(Filepath,"/Classification_Data/no-co-wild-phase-1-_1Apr_classifications.csv")
```
####Examine data
```{r}
...
Now Save our result! Save to the **Flattened_files** folder in the **Post_zooniverse** folder.
```{r}
write.csv(Flattened, "no-co-wild-phase-1-_1Apr_classifications-flat.csv", row.names = F)
```
